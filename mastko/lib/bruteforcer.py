...
class Bruteforcer:
    ...
    def __init__(self, targets: Sequence[Target]):
        ...
        self.eip = EIP(
            allocation_id=os.environ['MASTKO_EIP_ALLOCATION_ID'],
            ip_address=os.environ['MASTKO_EIP']
        )
    ...
    def _cycle_eip_through_autoscaling_group(self, autoscaling_group_name: str) -> None:
        try:
            ...
            message = (
                f"Failed to cycle eip through autoscaling group: {autoscaling_group_name}. ERROR: {err}"
            )
            log.error(message)
    ...
    def _check_if_takeover(self, ip_to_ec2_dict: list) -> list:
        for ip, instance_id in ip_to_ec2_dict.items():
            if ip in self.target_hash:
                log.info(f"SUCESSFULL ATTEMPT, takeover match found. ec2_used: {instance_id}, public_ip: {ip}")
